...
async function addImagesToAtlas() {
  try {
    ...
    for (let i = 0; i < data.questions.length; i++) {
      ...
      if (questionImages[questionNumber]) {
        ...
        if (validImages.length > 0) {
          ...
          if (!existingQuestion) {
            console.log(`⚠️  Question ${questionNumber} not found in MongoDB Atlas`);
          ...
          if (updateResult.matchedCount > 0) {
            if (updateResult.modifiedCount > 0) {
              console.log(`✅ Updated question ${questionNumber} in Atlas with ${validImages.length} image(s)`);
              if (validImages.length > 1) {
    ...
    if (skippedCount > 0) {
      console.log(`ℹ️  Skipped ${skippedCount} questions (not found or already have images)`);
    }

  } catch (error) {
    console.error('❌ Error adding images to MongoDB Atlas:', error);
    throw error;
  } finally {
